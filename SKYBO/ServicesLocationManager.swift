...
class LocationManager: NSObject, ObservableObject {
    ...
    private func setupLocationManager() {
        ...
        print("📍 Location Manager Setup - Accuracy: Best, Distance Filter: \(distanceFilter)m")
    }
...
extension LocationManager: CLLocationManagerDelegate {
    ...
    nonisolated func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        // Validate location freshness (reject if older than 30 seconds)
        let locationAge = abs(location.timestamp.timeIntervalSinceNow)
        guard locationAge < 30 else {
            print("⚠️ Rejected stale location (age: \(Int(locationAge))s)")
            return
        }
        
        // Validate accuracy (reject if worse than 100m)
        guard location.horizontalAccuracy > 0 && location.horizontalAccuracy < 100 else {
            print("⚠️ Rejected inaccurate location (accuracy: \(location.horizontalAccuracy)m)")
            return
        }
        
        // Log the location to help debug
        print("📍 Location Update: \(location.coordinate.latitude), \(location.coordinate.longitude)")
        print("   Accuracy: \(location.horizontalAccuracy)m, Age: \(Int(locationAge))s")
