    ...
    func loadUserData(userId: String) async {
        ...
        do {
            ...
            if let data = document.data() {
                print("📄 Raw Firestore document data:")
                if let roles = data["roles"] {
                    print("   roles field: \(roles) (type: \(type(of: roles)))")
                }
                if let role = data["role"] {
                    print("   role field: \(role) (type: \(type(of: role)))")
                }
            }
            ...
            print("🔵 User role (legacy): \(currentUser?.role.rawValue ?? "unknown")")
            print("🔵 User roles (array): \(currentUser?.roles.map { $0.rawValue } ?? [])")
            print("🔵 User primaryRole: \(currentUser?.primaryRole.rawValue ?? "unknown")")
